...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM academic_list where id = ".$_GET['id'])->fetch_assoc(); // fetch_array
	foreach($qry as $k => $v){
		$$k = $v;
	}
}
...
<script>
    ...
    console.log(formData);  // Log form data to verify radio button values are included
    $.ajax({
        ...
        success:function(resp){
            if(resp == 1){
                ...
                alert_toast('There are no changes', "error");
                end_load();
            }
        },
        error: function(xhr, status, error) {
            console.error(xhr.responseText);
            alert_toast('Error occurred while saving order', "error");
            end_load();
        }
    });
});
