    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == UPLOAD_ERR_OK) {
        ...
        if ($fileType == "csv") {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
                ...
                $stmt = $conn->prepare("INSERT INTO faculty_list (school_id, firstname, lastname, email, password, avatar, date_created) VALUES (?, ?, ?, ?, ?, ?, ?)");
                ...
                while (($data = fgetcsv($file, 1000, ",")) !== FALSE) {
                    ...
                    $uniqueIdentifier = $sanitizedData[3]; // Assuming email is the unique identifier

                    // Check if the row already exists in the database
                    $existingRowQuery = $conn->prepare("SELECT COUNT(*) FROM faculty_list WHERE email = ?");
                    ...
                    // Bind parameters
                    $stmt->bind_param("sssssss", $school_id, $firstname, $lastname, $email, $password, $avatar, $date_created);
